...
public final class DistanceFunctions {
	...
	public static double helinger(EuclideanCoordinate coord1, EuclideanCoordinate coord2) {
		int size = Math.min(coord1.dimensions(), coord2.dimensions());
		double distance = 0;
		for(int i = 0; i < size; i++) {
			double diff = Math.sqrt(coord1.get(i)) - Math.sqrt(coord2.get(i));
			distance += diff * diff;
		}
		distance = Math.sqrt(0.5*distance);
		return distance;
	}
	...
	public static final DistanceFunction<EuclideanCoordinate> EUCLIDEAN = new DistanceFunction<DistanceFunctions.EuclideanCoordinate>() {
		...
		public double calculate(EuclideanCoordinate coord1, EuclideanCoordinate coord2) {
			if(Constants.metric.equals("HELINGER"))return DistanceFunctions.helinger(coord1, coord2);
