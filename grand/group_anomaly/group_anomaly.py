...
class GroupAnomaly:
    ...
    w_ref_group : string
        ...
        Possible values for the units can be found in https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.to_
        delta.html
    ...
    def __init__(self, nb_units, ids_target_units, w_ref_group="7days", w_martingale=15, non_conformity="median", k=20,
                 dev_threshold=.6, transformer=None, w_transform=30, columns=None,R=1.5,thresOut=5,thresIn=30,metacheck=False,PH=60):
        ...
        self.detectors = [ IndividualAnomalyInductive(w_martingale, non_conformity, k, dev_threshold,R=R,thresOut=thresOut,thresIn=thresIn,metacheck=metacheck) for _ in range(nb_units) ]
    ...
    def plot_deviations(self, figsize=None, savefig=None, plots=["data", "transformed_data", "strangeness", "deviation", "threshold"], debug=False):
        ...
        nb_axs += len(self.ids_target_units) -1
        if "data" in plots:
            nb_axs += 1
        ...
        if any(s in ["pvalue", "deviation", "threshold"] for s in plots):
            ...
            for uid in self.ids_target_units:
                ...
                datesout=[]
                axs[i].set_ylabel(busses[uid])
                axs[i].set_ylim(0, 1)
                T, P, M , isDev = self.detectors[uid].T, self.detectors[uid].P, self.detectors[uid].M , self.detectors[uid].isDev
                if "pvalue" in plots:
    ...
    def plot_extra_outliers(self,T,bus,ax):
        ...
        with open(path+"redouliers.txt", 'r') as f: 
            ...
            for line in Lines: 
                split = line.split(" ")
                if split[0]==bus :
                    dateRed= datetime.datetime.strptime(split[1].strip(), '%Y-%m-%d') 
        ...
        IGNORE=[]
        for d in T:
            cl = self.isFPFN(d,outlier,onlyforcheck)
            if cl==1:
                TP.append(d)
            elif cl==0 :
                FP.append(d)
                ...
                IGNORE.append(d)
        unred,unblue = self.CalculateUncatchOutliers(T,outlier,onlyforcheck)
        print("Buss"+bus+" TP : "+str(len(TP))+" FP : "+str(len(FP))+"  Red Uncatch/All: "+str(unred)+"/"+str(len(onlyforcheck))+"  Blue Uncatch/All: "+str(unblue)+"/"+str(len(outlier)))
    ...
    def distfromRedOutlier(self,date,redoutliers):
        PH=self.PH
        for redoutlier in redoutliers:
    	    if redoutlier < date and abs((redoutlier - date).days)<PH :
                return 1
        return 0
    def isFPFN(self,date,outliers,redoutliers):
        PH=self.PH
        for redoutlier in redoutliers:
            if date < redoutlier:
                diff =(redoutlier - date).days
                if diff < PH :
                    return 1
        for d in outliers:
            if date < d:
                ...
                if diff < PH :
                    return 1
                elif self.distfromRedOutlier(date,redoutliers)==-1 :
                    return -1
                else :
                    return 0
        if self.distfromRedOutlier(date,redoutliers) == -1 :
            return -1
        return 0
    def CalculateUncatchOutliers(self,y,outliers,redoutliers):
        PH=self.PH
        out = outliers.copy()
        red = redoutliers.copy()
        for date in y:
            for o in out:
                diff=(o-date).days
                if diff>0 and diff<PH:
                    out.remove(o)
            for o in red:
                diff=(o-date).days
                if diff>0 and diff<PH:
                    red.remove(o)
        return len(red),len(out)    
