...
class GroupAnomaly:
    ...
    def __init__(self, nb_units, ids_target_units, w_ref_group="7days", w_martingale=15, k=20,
    ...
    def predict(self, dt, x_units):
        ...
        count=0
        for x in x_units:
            data_naive = Data_naive(dt,x)
            data_naive.flag=0
            self.CurrentData.append((count,data_naive))
        
        for x in self.CurrentData:
            if x[1].flag==1:
                self.CurrenData.remove(x)
...
class Data_naive:
    def __init__(self,dt,x):
        ...
        self.count_after=0
        self.nn_before=[]
        self.safe_inlier=False
        
        def insert_nn_before(self,el,k):
            if k!=0 and len(self.nn_before) ==k :
                tmp=min(self.nn_before)
                if el < tmp :
                    self.nn_before.remove(tmp)
                    self.nn_before.append(el)
                else :
                    self.nn_before.append(el)
            
        def get_min_nn_before(time):
            c=0
            temp=[]
            for x in self.nn_before:
                if x>=time:
                    temp.append(x)
                    c+=1
            if c==0:
